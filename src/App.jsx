...
const saveData = async (key,val) => { try { await window.storage.set(key,JSON.stringify(val)); } catch(e){} };
...
const INGREDIENT_LIBRARY = [
  ...
  {name:"GF Arnotts Biscuits",measure:"kg",contentsQty:1440,contentsUnit:"per gram",unitCost:56.00/1440},
  ...
  {name:"Brown Rice",measure:"kg",contentsQty:10000,contentsUnit:"per gram",unitCost:29.50/10000},
  ...
  {name:"Jalapeno Sausages",measure:"kg",contentsQty:1000,contentsUnit:"per gram",unitCost:20.00/1000},
  ...
  {name:"Parmesan Bagels",measure:"each",contentsQty:1,contentsUnit:"each",unitCost:1.70},
  ...
  {name:"Eggplant Strips",measure:"kg",contentsQty:2000,contentsUnit:"per gram",unitCost:36.00/2000},
  ...
  {name:"Pickled Cabbage",measure:"kg",contentsQty:1000,contentsUnit:"per gram",unitCost:6.00/1000},
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const d = await loadData(STORAGE_KEYS.diary, initDiary());
      const b = await loadData(STORAGE_KEYS.biz, initBiz());
      const r = await loadData(STORAGE_KEYS.recipes, []);
              ...
              return(
                <div style={{background:WHITE,borderRadius:12,padding:24}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(120px, 1fr))",gap:10,marginBottom:24}}>
                    {[{label:"TOTAL COST",value:fmtMoney(totalCost),color:TEXT},{label:"COST PER SERVE",value:fmtMoney(costPerServe),color:TEXT},{label:"SELL PRICE",value:fmtMoney(sellPrice),color:TEXT},{label:"SELL EX GST",value:fmtMoney(sellExGst),color:TEXT},{label:"FOOD COGS %",value:sellPrice>0?`${cogs.toFixed(1)}%`:"—",color:cogsColor(cogs)},{label:"GP %",value:sellPrice>0?`${gp.toFixed(1)}%`:"—",color:gpColor(gp)}].map(({label,value,color})=>(
                  ...
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                      <thead><tr style={{background:OLIVE_LIGHT}}>{["Ingredient","Measure","Single Qty Unit","Single Serve","Portion Qty","Actual","Unit Cost ($)","Total Cost"].map((h,i)=>(<th key={h} style={{padding:"8px",textAlign:i===0?"left":"right",fontSize:11,color:OLIVE,fontWeight:"bold",whiteSpace:"nowrap"}}>{h}</th>))}<th style={{width:30}}></th></tr></thead>
                      <tbody>
